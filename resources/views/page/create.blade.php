...
@section('title', trans('page.page_title.create'))

@section('content')

<h1>{{ trans('page.page_title.create') }}</h1>

<form method="post" action="{{ action('PageController@store') }}" class="vertical-form">
	{{ csrf_field() }}
	...
	<div class="vertical-form-row button-row">
		<div class="field-column">
			<input class="button" type="submit" value="{{ trans('page.button.submit.create') }}">
		</div>
	</div>
</form>
